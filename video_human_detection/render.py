...
class OpenCVRenderer:
    ...
    def __init__(
        ...
        self.fps = fps
        self.codec = codec
        # self.writer = cv2.VideoWriter(outfile, cv2.VideoWriter_fourcc(*codec), fps, self.shape)
        self.writer = None

    def add_frame(self, frame: np.ndarray):
        """Write the next frame to the video file. All frames must of the same shape, and have (H, W, 3) format.

        :param frame: Frame in (H, W, 3) format.
        ...
        if self.writer is None:
            ...
            self.writer = cv2.VideoWriter(
                self.outfile, cv2.VideoWriter_fourcc(*self.codec), self.fps, self.shape
            )
